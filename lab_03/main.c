...
int interp_spline(table_r table, double x_arg)
{
    ...
    double arr_a[table.count];
    double arr_b[table.count];
    double arr_d[table.count];
    double arr_c[table.count];
    double arr_F[table.count];
    double arr_Kci[table.count];
    double arr_Etta[table.count];
    double arr_Hn[table.count];
    ...
    for (int i = 2; i < table.count; ++i)
    {
        arr_F[i] = (3) * ((table.y_arr[i] - table.y_arr[i - 1]) / arr_Hn[i] - 
                           (table.y_arr[i-1] - table.y_arr[i - 2]) / arr_Hn[i - 1]);
        arr_Kci[i + 1] = - arr_Hn[i] / (arr_Hn[i-1] * arr_Kci[i] + 2 * (arr_Hn[i-1] + arr_Hn[i]));
        arr_Etta[i + 1] = (arr_F[i]-arr_Hn[i-1] * arr_Etta[i])/(arr_Hn[i-1] * arr_Kci[i] + 2 * (arr_Hn[i-1] + arr_Hn[i]));
    }
    // Обратный ход (2 этап)
